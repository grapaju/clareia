	...
	collection(name) {
		...
		return {
			...
			async create(payload) {
				if (collectionName === 'users') {
					...
					if (result?.token && result?.user) {
						apiClient.authStore.save(result.token, result.user);
					}
			...
			async authRefresh() {
				...
				if (result?.user) {
					apiClient.authStore.save(getStoredToken(), result.user);
				}
				return {
					record: result?.user,
...
function getCurrentAccountId() {
	return String(apiClient?.authStore?.model?.currentAccountId || '').trim();
}
