...
import { BatteryMedium, CheckCircle2, MoreHorizontal, Pencil, Play, RotateCcw, Trash2, Archive, Clock3 } from 'lucide-react';
...
export default function TodayTaskRow({ task, isRecommended = false, completed = false, workedMinutes = 0, onStart, onOpen, onEdit, onComplete, onReopen, onWaiting, onArchive, onDelete }) {
  ...
  return (
    <li className="group content-fade-in flex min-h-14 items-center gap-3 border-b border-border/70 px-4 py-3 last:border-b-0">
      {completed ? (
        <CheckCircle2 className="task-complete-pop h-4 w-4 shrink-0 text-muted-foreground/70" aria-hidden="true" />
      ) : (
        <span className={`h-2.5 w-2.5 shrink-0 rounded-full ${isRecommended ? 'bg-sky-400' : status === TASK_STATUS.EM_ANDAMENTO ? 'bg-primary' : 'border border-muted-foreground/50'}`} aria-hidden="true" />
      )}
      <button type="button" onClick={() => onOpen(task)} className="min-w-0 flex-1 rounded-sm text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
        ...
        <span className="mt-0.5 flex flex-wrap gap-x-2 text-xs text-muted-foreground">
          ...
          {metadata.minutes > 0 && <span className="inline-flex items-center gap-1"><Clock3 className="h-3 w-3" aria-hidden="true" />{formatDurationFriendly(metadata.minutes)}</span>}
          <span>{metadata.situation === 'Atrasada' ? 'Para reorganizar' : metadata.situation}</span>
          {metadata.isRoutine && <span>Rotina</span>}
          {(task.energiaNecessaria || task.energyLevel) && <span className="inline-flex items-center gap-1"><BatteryMedium className="h-3 w-3" aria-hidden="true" />{task.energiaNecessaria || task.energyLevel} energia</span>}
          {metadata.progress && <span>{metadata.progress} passos</span>}
          {completed && workedMinutes > 0 && <span>{formatDurationFriendly(workedMinutes)} realizados</span>}
      ...
      {completed ? (
        ...
        <>
          <Button size="sm" className="press-feedback shrink-0" onClick={() => onStart(task)}><Play className="mr-1.5 h-4 w-4" aria-hidden="true" /> {isPaused ? 'Continuar' : 'Começar'}</Button>
          <DropdownMenu>
