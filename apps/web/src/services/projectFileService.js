...
export function createProjectFile(payload) {
  ...
  const item = {
    ...
    uploadReceiptId: String(payload.uploadReceiptId || '').trim(),
    uploadStatus: String(payload.uploadStatus || '').trim(),
...
export function updateProjectFile(id, updates = {}) {
  ...
  const updated = {
    ...
    content: updates.content !== undefined ? String(updates.content || '').trim() : String(current.content || '').trim(),
    ...
    mimeType: updates.mimeType !== undefined ? String(updates.mimeType || '').trim() : String(current.mimeType || '').trim(),
    size: updates.size !== undefined ? Number(updates.size) || 0 : Number(current.size) || 0,
    uploadReceiptId: updates.uploadReceiptId !== undefined ? String(updates.uploadReceiptId || '').trim() : String(current.uploadReceiptId || '').trim(),
    uploadStatus: updates.uploadStatus !== undefined ? String(updates.uploadStatus || '').trim() : String(current.uploadStatus || '').trim(),
